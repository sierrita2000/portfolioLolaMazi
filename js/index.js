...
const cargarEventosCabecera = () => {
  ...
  window.addEventListener('scroll', () => {
    ...
    (window.scrollY > 0) ? cabecera_menu.style.backgroundColor = '#0d0d0d85' : cabecera_menu.style.backgroundColor = 'transparent'
    
    scroll_anterior = window.scrollY;
  })
  
  // función para añadir el handler que controla si debe aparecer el menu hamburguesa. 
  desplegarMenu(circulo__hamburguesa, hamburguesa__icono, cabecera_menu, menu, boton_hamburguesa, cabecera);
  ...
  menu.forEach((boton) => {
    boton.addEventListener("click", () => {
      ...
      if (cabecera_menu.classList.contains('cabecera__menu__desplegado')){
        abrir_cerrar_menu(circulo__hamburguesa, hamburguesa__icono, cabecera_menu, menu, boton_hamburguesa, cabecera, body);
      }
...
const cargarEventosHome = () => {
  ...
  fotos.forEach((foto) => {
    ...
    foto.addEventListener("click", () => {
      if (foto.classList.contains("mostrar")) {       // la foto está ya en le frente super puesta al resto.
        fotos.forEach((foto) => {
          foto.classList.add("foto_animation");
        });
        foto.classList.remove("mostrar");
      } else {                                        // pone la foto al frente del resto.
        fotos.forEach((foto) => {
...
const cargarEventosAbout = () => {
  ...
  const elementos = [gustos_texto, circulo3, titulo3, texto3, circulo1, titulo1, texto1]
  ...
  boton_arriba.addEventListener('click', () => {
    if (distancia != 0) {                                             // si se puede seguir pulsando porque hay más elementos.
 ...
 */
const cargarEventosProjects = () => {
  // Elementos del DOM
  const concierto__cintas = document.querySelectorAll('.concierto__cinta');
  const concierto__fotos__concierto2 = document.querySelector('.concierto__fotos__concierto2');
  const concierto__fotos__concierto3 = document.querySelector('.concierto__fotos__concierto3');
  const concierto__fotos__concierto4 = document.querySelector('.concierto__fotos__concierto4');
  ...
  const icono_cronicas = document.querySelector('.iconos_videos_fotos__cronicas');
  const titulo_cronicas = document.querySelector('.seccion_cronicas__titulo .cronicas_videos_fotos__titulo');
  ...
  const aparecer_cintas = [concierto__cintas[0], concierto__cintas[1], concierto__cintas[2], concierto__cintas[3], icono_cronicas, boton_izquierda, boton_derecha];
  ...
  window.addEventListener('scroll', () => {
    ...
    const [ top_concierto, offset_concierto, height_concierto ] = medidas(concierto__fotos__concierto2);
    ...
    const [ top_concierto3, offset_concierto3, height_concierto3 ] = medidas(concierto__fotos__concierto3);
    ...
    const [ top_concierto4, offset_concierto4, height_concierto4 ] = medidas(concierto__fotos__concierto4);
  ...
  const fotos_a_mover = [[fotos[0], fotos[2], fotos[3], fotos[5], fotos[6], fotos[8], fotos[9], fotos[11], fotos[12]], [fotos[14], fotos[16], fotos[17], fotos[19], fotos[20]], [fotos[22], fotos[24], fotos[25], fotos[27], fotos[28], fotos[30]], [fotos[31], fotos[33], fotos[34], fotos[36], fotos[37], fotos[39], fotos[40]]];
  ...
  conjunto_fotos.forEach(fotos => {
    eventosSlider(fotos, fotos_a_mover[posicion_array]); // función que controla si se desplaza el slider.
    posicion_array++;
  })
 ...
 */
const desplegarMenu = (circulo__hamburguesa, hamburguesa__icono, cabecera_menu, menu, boton_hamburguesa, cabecera) => {
 ...
 */
const abrir_cerrar_menu = (circulo__hamburguesa, hamburguesa__icono, cabecera_menu, menu, boton_hamburguesa, cabecera, body) => {
...
const moverFotografias = (movimiento_anterior, movimiento, fotos_a_mover) => {
  if (movimiento_anterior > movimiento) {               // si se desplaza el slider a la derecha.
    fotos_a_mover.forEach(foto => {
      foto.classList.add('desplazar_foto_izq');
      foto.classList.remove('desplazar_foto_der');
    })
  } else {                                              // si se desplaza el slider a la izquierda.
    fotos_a_mover.forEach(foto => {
